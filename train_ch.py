...
weight_path = "./Cache/epoch_10_ch.pth"
...
def load_data(batch_size,features,num_steps=50,train_vocab=None,test_vocab=None):
    ...
    train_set = DataProcess.FakeNewsDataset(train_data,num_steps,train_vocab)
    test_set = DataProcess.FakeNewsDataset(test_data,num_steps,test_vocab)
...
if __name__ == "__main__":
    ...
    if train:
        ...
        for i in tqdm(range(len(test_data[0]))):
            label = Module.evalScript.predict_fake_news(net, vocab, jieba.lcut(test_data[0][i]), jieba.lcut(test_data[1][i]))
