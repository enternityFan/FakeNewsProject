...
weight_path = "./Cache/epoch10_en_cnn_v3.pth"
...
for i in tqdm(range(len(test_data[0]))):
    label = Module.evalScript.predict_fake_news_cnn(net, vocab, test_data[1][i].split(), test_data[2][i].split())
...
df.to_csv("./Data/submission_epoch10_en_cnn_v3.csv",index=False)
