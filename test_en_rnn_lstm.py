...
import pickle
import re
import Module.LSTMRNNModel

weight_path = "./Cache/epoch10_en_lstm_1.pth"
...
#net = Module.RNNModel.RNNModel(vocab,embed_size,num_hiddens,mode='predict')
net = Module.LSTMRNNModel.LSTMRNNModel(vocab,embed_size,num_hiddens,mode='predict')
...
for i in tqdm(range(len(test_data[0]))):
    ...
    label = Module.evalScript.predict_fake_news_rnn(net, vocab, test_data[1][i].split(), test_data[2][i].split())
...
df.to_csv("./Data/submission_epoch10_en_lstm_1.csv",index=False)
