...
### AttentionModel_2
修改激活函数为PReLu()，修改MLP层数为3，把隐藏单元数量改为200，同时增大dropout的概率。
...
注：没有特殊说明，实验结果都是在迭代10次得到的。在模型参数计算时，未计算embedding层参数。

计算RNN模型时，需要注意W~hq~，b~q~,W~hh~,b~h~,W~xh~,这是每一个H含有的权重。这里q就是输入的维度x（x是embed_size），计算出来一个H的参数后，再乘一个词元数就行。

| Model               | Private Score | Public Score | 描述                                                         | 模型参数计算 |
| ------------------- | ------------- | ------------ | ------------------------------------------------------------ | :----------: |
| Attention_Model_ch  | 0.48510       | 0.52344      | 用的中文的数据做的，不过后来因为分词太慢了，并且效果也很垃圾。。使用的模型就再也不处理中文的了。 |   ≈442.6K    |
| Attention_Model_en  | 0.62478       | 0.58261      | 使用的英文的数据进行的。                                     |   ≈402.6K    |
| Attention_schaduler | 0.65485       | 0.62512      | 这个就是在上面的基础上添加了学习率调度器，效果还是提升了不少的。 |   ≈402.6K    |
| AttentionModel_2    | **0.69389**   | **0.72253**  | 这个是训练了30次得到的结果。主要修改了部分网络的结构。       |   ≈603.6K    |
| RNN_Model           | 0.51205       | 0.55162      | 这个结果还是很差的，并且有一个很麻烦的问题，不过也可能是我batch_size设置的太大了，导致前几批次loss不下降，并且修改lr后会10次迭代loss都不下降。这个模型只用了一层的RNN，就这模型参数都这么多。。 |   =8190.6K   |
...
![å¨è¿éæå¥å¾çæè¿°](F:\Study\NLP\FakeNewsProject\README.assets\watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3BlcnNpc3RpbmxpZmU=,size_16,color_FFFFFF,t_70.png)
